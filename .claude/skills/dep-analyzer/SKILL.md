...
- `Depth` / `--depth`: recursive depth for graph traversal (default: 3).
- `OutFormat` / `--out-format`: `text` | `json` | `md`.
- `Limit` / `--limit`: page size (default: 150).
- `Offset` / `--offset`: page offset (default: 0).
- `OutFile` / `--out-file`: optional path to write output.
- `Direction` / `--direction`: traversal direction `outgoing` | `incoming` | `both` (default: both).
