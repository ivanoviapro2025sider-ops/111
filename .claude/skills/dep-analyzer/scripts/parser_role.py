"""Парсер ролей: Rights.xml, RLS, шаблоны ограничений."""
...
def _guess_role_xml(role_dir: Path, role_name: str) -> Optional[Path]:
    """Определить основной XML-файл роли (Name.xml или Rights.xml)."""
    candidates = [
...
def _parse_object_rights(root) -> List[ObjectRights]:
    """Парсинг элементов Right, группировка по объектам, извлечение RLS-условий."""
    result: List[ObjectRights] = []

    # Пробуем найти элементы Right по local-name
    right_nodes = get_xml_elements(root, ".//*[local-name()='Right']")
    # Фолбэк через namespace roles
    if not right_nodes:
    ...
    for node in right_nodes:
        ...
        has_rls = _to_bool(
            get_xml_text(node, ".//*[local-name()='UseRestrictionByCondition']"),
            default=False,
        )
        rls_condition = (
...
def _parse_rls_templates(root) -> List[RLSTemplate]:
    """Парсинг шаблонов ограничений (Template)."""
...
def parse_roles(graph) -> None:
    """Парсинг всех ролей в графе: права, RLS-условия, шаблоны."""
    for obj in graph.objects.values():
        ...
        role = RoleInfo(
            ...
            set_for_new_objects=_to_bool(
                get_xml_text(root, ".//*[local-name()='SetForNewObjects']")
            ),
            set_for_attributes_by_default=_to_bool(
                get_xml_text(root, ".//*[local-name()='SetForAttributesByDefault']", default="true"),
                default=True,
            ),
            independent_rights=_to_bool(
                get_xml_text(root, ".//*[local-name()='IndependentRights']")
            ),
