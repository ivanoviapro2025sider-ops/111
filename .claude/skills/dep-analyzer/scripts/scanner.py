...
def _iter_object_dirs(base_path: Path) -> Iterable[Tuple[str, Path]]:
    """Перебор каталогов объектов по маппингу TYPE_TO_FOLDER."""
...
def _guess_main_xml(object_dir: Path, object_name: str) -> Optional[Path]:
    """Определить основной XML-файл объекта метаданных."""
    preferred = [
    ...
    if ext_dir.is_dir():
        ext_files = sorted(ext_dir.glob("*.xml"))
        if ext_files:
            return ext_files[0]
    return None
...
def _collect_list(root, xpath: str) -> List[str]:
    """Собрать список уникальных текстовых значений по XPath."""
    result: List[str] = []
...
def _fill_basic_fields(obj: ObjectInfo, main_xml_path: Optional[Path]) -> None:
    """Заполнить базовые поля объекта: synonym, comment, forms, templates, commands."""
...
def scan_configuration(config_path: str) -> DependencyGraph:
    """Построить начальный индекс объектов, сканируя каталоги выгрузки конфигурации."""
...
def build_object_xml_index(graph: DependencyGraph) -> Dict[str, str]:
    """Построить маппинг ключ метаданных → путь к основному XML."""
    index: Dict[str, str] = {}
...
def read_configuration_root(config_path: str):
    """Парсинг верхнеуровневого Configuration.xml, если он существует."""
