...
def _paginate(items: List[dict], limit: int, offset: int) -> List[dict]:
    """Универсальная пагинация списка."""
    if offset < 0:
        offset = 0
    if limit <= 0:
        return items[offset:]
...
def _build_edges_payload(graph: DependencyGraph, target: str, depth: int, direction: str) -> List[dict]:
    """Построить список рёбер для вывода."""
    if target:
...
def render_deps(
    ...
    """Отрендерить вывод режима deps в указанном формате."""
