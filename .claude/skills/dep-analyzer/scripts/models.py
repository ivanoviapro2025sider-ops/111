...
class EdgeKind(str, Enum):
    """Edge kinds for dependency graph."""
...
@dataclass
class AttributeInfo:
    """Attribute, dimension or resource information."""
...
@dataclass
class ReferenceInfo:
    """Reference to other metadata object."""
...
@dataclass
class BSLProcedure:
    """Procedure/function parsed from BSL module."""
...
@dataclass
class BSLQueryRef:
    """Metadata reference parsed from query text."""
...
@dataclass
class RightInfo:
    """Single right on metadata object."""
...
@dataclass
class ObjectRights:
    """Rights set for one metadata object."""
...
@dataclass
class DebugPoint:
    """Potential breakpoint for debugging."""
...
@dataclass
class Edge:
    """Graph edge between two objects."""
...
class DependencyGraph:
    """In-memory dependency graph for metadata and BSL edges."""
    ...
    def traverse(self, start: str, depth: int, direction: str = "outgoing") -> List[Tuple[Edge, int]]:
        """Traverse graph from start up to depth.

        direction: outgoing | incoming | both
        Returns list of tuples: (edge, level), where level starts from 1.
        """
        if depth <= 0:
            return []
        ...
        queue = deque([(start, 0)])
        visited_level: Dict[str, int] = {start: 0}
        emitted_edges: set[Tuple[str, str, EdgeKind, int]] = set()
        ...
        while queue:
            ...
            if direction in {"outgoing", "both"}:
                current_edges.extend(self.get_edges_from(current))
            if direction in {"incoming", "both"}:
                current_edges.extend(self.get_edges_to(current))

            for edge in current_edges:
                edge_signature = (edge.source, edge.target, edge.kind, level + 1)
                if edge_signature not in emitted_edges:
                    emitted_edges.add(edge_signature)
                    result.append((edge, level + 1))

                next_node = edge.target if edge.source == current else edge.source
                next_level = level + 1
                best_known = visited_level.get(next_node)
                if next_level <= depth and (best_known is None or next_level < best_known):
                    visited_level[next_node] = next_level
