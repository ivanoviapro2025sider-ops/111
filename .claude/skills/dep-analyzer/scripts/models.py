"""Общие модели данных для dep-analyzer: типы объектов, граф зависимостей, права."""
...
class DependencyGraph:
    ...
    def traverse(self, start: str, depth: int, direction: str = "outgoing") -> List[Tuple[Edge, int]]:
        """BFS-обход графа от start до глубины depth.
    ...
    def add_role(self, role_info: RoleInfo):
        self.roles[role_info.name] = role_info

    def get_all_objects_of_type(self, obj_type: str) -> List[ObjectInfo]:
        """Получить все объекты указанного типа."""
        return [o for o in self.objects.values() if o.obj_type == obj_type]

    def get_connected_objects(self, key: str, direction: str = "both") -> Set[str]:
        """Получить множество связанных объектов (ключей)."""
        result: Set[str] = set()
        if direction in ("outgoing", "both"):
            for e in self.get_edges_from(key):
                result.add(e.target)
        if direction in ("incoming", "both"):
            for e in self.get_edges_to(key):
                result.add(e.source)
        return result
