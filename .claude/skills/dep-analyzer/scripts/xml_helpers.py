...
def _detect_nsmap(root) -> dict:
    """Автоматически определить правильную карту namespace на основе корневого элемента."""
    if root is None:
        return NSMAP
...
def get_xml_text(element, xpath: str, default: str = "", nsmap: dict = None) -> str:
    ...
    if ns is NSMAP:
        try:
            found = element.xpath(xpath, namespaces=NSMAP_ALT)
            if found:
                if hasattr(found[0], 'text'):
                    return (found[0].text or default).strip()
                return str(found[0]).strip() or default
...
def get_xml_elements(element, xpath: str, nsmap: dict = None) -> list:
    ...
    if ns is NSMAP:
        try:
            return element.xpath(xpath, namespaces=NSMAP_ALT)
        except Exception:
            pass
    return []
...
_PRIMITIVE_TYPES = {
    ...
    "v8:UUID", "v8:ValueStorage", "v8:Null",
}


def parse_type_value(type_text: str) -> Optional[Dict[str, str]]:
    """Парсинг текстового значения типа. Возвращает {'type': ..., 'name': ..., 'full_type': ...} или None."""
    if not type_text:
        return None

    text = type_text.strip()

    if text in _PRIMITIVE_TYPES:
        return None
    ...
    m = re.match(r"(?:cfg:)?DefinedType\.(\w+)", text)
    if m:
        return {
            "type": "DefinedType",
            "name": m.group(1),
            "full_type": f"DefinedType.{m.group(1)}",
        }
...
def parse_types_from_element(type_container) -> List[Dict[str, str]]:
    """Извлечь все типы из контейнера <Type> (может содержать несколько v8:Type)."""
    results: List[Dict[str, str]] = []
    if type_container is None:
        return results

    type_elements = get_xml_elements(type_container, "v8:Type")
    if not type_elements:
        type_elements = get_xml_elements(type_container, "Type")

    for te in type_elements:
        text = te.text if hasattr(te, 'text') and te.text else ""
