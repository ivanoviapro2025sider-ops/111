...
def render_debug(
    ...
    if out_format == "json":
        return json.dumps(
            {"mode": "debug", "target": target, "total": len(points), "count": len(payload), "items": payload},
            ensure_ascii=False,
            indent=2,
        )
    ...
    if out_format == "md":
        ...
        for item in payload:
            lines.append(
                f"| `{item['procedure_name']}` | `{item['module_path']}` | {item['line_number']} | {item['context']} |"
            )
        return "\n".join(lines)

    lines = [f"Mode: debug", f"Target: {target or 'ALL'}", f"Total points: {len(points)}", ""]
    for item in payload:
        lines.append(
