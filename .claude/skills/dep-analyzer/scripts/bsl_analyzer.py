"""Статический анализ BSL: процедуры, вызовы, обращения к метаданным, запросы."""
...
# Объявления процедур/функций (русский + английский)
PROC_RE = re.compile(
...
# Ссылки на метаданные в текстах запросов
...
def _read_lines(path: Path) -> List[str]:
    """Чтение BSL-файла (UTF-8 с фолбэком на CP1251)."""
    try:
...
def _iter_bsl_files(object_path: str) -> Iterable[Path]:
    """Рекурсивный поиск .bsl файлов в каталоге объекта."""
...
def analyze_bsl(graph) -> None:
    """Статический анализ BSL-кода всех объектов графа.

    Заполняет: procedures, bsl_calls, bsl_query_refs в каждом ObjectInfo.
    """
    # Определяем корень конфигурации для относительных путей
    ...
    for key, obj in graph.objects.items():
        ...
        for bsl_file in _iter_bsl_files(obj.path):
            ...
            for idx, line in enumerate(lines, start=1):
                ...
                # Обращения к менеджерам метаданных
                ...
                # Ссылки на метаданные в текстах запросов (строковые литералы)
