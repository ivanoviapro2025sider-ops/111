"""Entry-point and CLI for dep-analyzer skill."""
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--config-path", required=True, dest="config_path", help="Path to 1C XML configuration export")
    parser.add_argument("--mode", default="deps", choices=sorted(ALLOWED_MODES), help="Output mode")
    parser.add_argument("--target", default="", help="Target object key, e.g. Document.Sales")
    parser.add_argument("--depth", type=int, default=3, help="Traversal depth for deps/full")
    parser.add_argument("--out-format", default="text", choices=sorted(ALLOWED_FORMATS), dest="out_format")
    parser.add_argument("--limit", type=int, default=150, help="Result row limit")
    parser.add_argument("--offset", type=int, default=0, help="Result row offset")
    parser.add_argument("--out-file", default="", dest="out_file", help="Output file path")
    parser.add_argument(
        "--direction",
        default="both",
        choices=sorted(ALLOWED_DIRECTIONS),
        help="Traversal direction for deps/full",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.out_file:
            ...
            print(output)
        return 0
    except Exception as exc:  # pragma: no cover - top-level guard for CLI robustness
