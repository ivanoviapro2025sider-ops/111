...
def _parse_attribute(attribute_node, *, tabular_section: str = "") -> AttributeInfo:
    return AttributeInfo(
        name=_text_of_first(attribute_node, "Name") or attribute_node.get("name") or attribute_node.get("Name") or "",
        types=_parse_types(attribute_node),
        tabular_section=tabular_section,
    )


def _iter_reference_infos(attribute: AttributeInfo) -> Iterable[ReferenceInfo]:
...
def _extract_registers_from_xml(root) -> List[str]:
    xml_text = etree.tostring(root, encoding="unicode")
    registers = set()
    for register_type in _REGISTER_TYPES:
        pattern = re.compile(
            rf"(?:cfg:)?{register_type}(?:RecordSet|RecordKey|Ref)?\.([A-Za-zА-Яа-яЁё0-9_]+)"
        )
        for match in pattern.finditer(xml_text):
            registers.add(f"{register_type}.{match.group(1)}")
    return sorted(registers)


def _extract_based_on(root) -> List[str]:
    based_on = set()
    for node in root.xpath(".//*[local-name()='BasedOn' or local-name()='BasedOnDocuments' or local-name()='InputByString']"):
        for type_info in parse_types_from_element(node):
            if type_info["type"] != "Document":
                continue
            based_on.add(type_info["name"])
    return sorted(based_on)


def parse_documents(object_index: Dict[str, ObjectInfo]) -> None:
    """Enrich document objects with attributes, references, and posting metadata."""
    ...
    for obj_info in object_index.values():
        ...
        attr_nodes = root.xpath(".//*[local-name()='Attribute' and not(ancestor::*[local-name()='TabularSection'])]")
        ...
        for tabular_node in root.xpath(".//*[local-name()='TabularSection']"):
            section_name = _text_of_first(tabular_node, "Name") or tabular_node.get("name") or tabular_node.get("Name") or ""
            if not section_name:
                continue
            section_attributes: List[AttributeInfo] = []
            for attr_node in tabular_node.xpath(".//*[local-name()='Attribute']"):
                attribute = _parse_attribute(attr_node, tabular_section=section_name)
                if not attribute.name:
                    continue
                section_attributes.append(attribute)
                for ref_info in _iter_reference_infos(attribute):
                    signature = (ref_info.source_attribute, ref_info.target_type, ref_info.target_name, ref_info.tabular_section, ref_info.ref_kind)
                    if signature not in ref_signatures:
                        ref_signatures.add(signature)
                        references.append(ref_info)
            tabular_sections.append(TabularSectionInfo(name=section_name, attributes=section_attributes))

        obj_info.attributes = attributes
        obj_info.tabular_sections = tabular_sections
        obj_info.references = references
        obj_info.based_on = _extract_based_on(root)
        obj_info.movement_registers = _extract_registers_from_xml(root)
