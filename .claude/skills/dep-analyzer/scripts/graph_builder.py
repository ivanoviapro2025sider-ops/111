"""Построение графа зависимостей + рекурсивный обход Depth."""
...
def _find_register_key(graph: DependencyGraph, register_name: str) -> Optional[str]:
    """Найти регистр по имени, пробуя все 4 типа регистров."""
...
def _edge_kind_for_reference(source_type: str, target_type: str, ref_kind: str) -> EdgeKind:
    """Определить тип ребра графа по типам источника, цели и характеру ссылки."""
    if ref_kind == "owner":
...
def build_edges(graph: DependencyGraph) -> None:
    """Построить рёбра графа на основе распарсенных метаданных и BSL-анализа."""
    # Сброс существующих рёбер
    graph.edges = []
    graph._edges_by_source = defaultdict(list)
    graph._edges_by_target = defaultdict(list)
    graph._edges_by_kind = defaultdict(list)

    for source_key, obj in graph.objects.items():
        # Ссылки из реквизитов
        ...
        if obj.obj_type == "EventSubscription":
            ...
            # Подписка → общий модуль-обработчик
            if obj.handler:
        ...
        # Ссылки на метаданные в BSL/запросах
...
def collect_subgraph(
    ...
    """Вернуть подграф как сериализуемый список с уровнями глубины."""
