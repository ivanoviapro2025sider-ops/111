...
async function extractSpreadsheetPreview(filePath: string) {
  ...
  const buffer = Buffer.from(await readFile(filePath));
  const loadWorkbook = workbook.xlsx.load.bind(workbook.xlsx);
  type WorkbookLoadBuffer = Parameters<typeof loadWorkbook>[0];
  const loadBuffer = buffer as unknown as WorkbookLoadBuffer;
  await loadWorkbook(loadBuffer);
